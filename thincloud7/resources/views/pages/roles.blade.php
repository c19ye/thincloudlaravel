...
@section('content')
    <div class="app-content content">
        ...
        @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <span>Toplam {{$number}} Kullanıcı</span>
                                ...
                                <div class="d-flex justify-content-between align-items-end mt-1 pt-25">
                                    ...
                                    <a href="javascript:;" title="{{$role->name}}" class="role-useradd-modal" data-bs-toggle="modal" id="{{$role->id}}"
                                        data-bs-target="#role_adduser{{$role->id}}"><i data-feather="user-plus"
                                            class="font-medium-5"></i></a>
                                    <a href="/roles/delete/{{$role->id}}" id="{{$role->id}}"
                ...
                <h3 class="mt-50">Rolleri ile toplam kullanıcı sayısı: {{$rolecount->count()}} </h3>
                <p class="mb-2">
...
>
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form action="/roles/adduser" method="POST" >
          @csrf
        <input name="role_id" type="hidden" value="{{$role->id}}" class="get_id">
      <div class="modal-body pb-5 px-sm-4 mx-50">
        <h1
          class="address-title text-center mb-1"
          id="createfullbackuptitle"
        >
          {{$role->name}} Yeni Kullanıcı Ekle
        </h1>
        <hr class="bg-primary" />
        <div class="row">
          <h2>Rol Kullanıcılarını Seç</h2>
          @foreach ($thinclouds as $thincloud)
          <div class="col-6">
          <input
          {{ $thincloud->role_id == $role->id ? 'checked' : '' }}
          class="form-check-input"
          type="checkbox"
          name="thincloud_id[]"
          value="{{$thincloud->id}}"
          id="vm{{$thincloud->id}}{{$role->id}}"
        />
        <label class="form-check-label" for="vm{{$thincloud->id}}{{$role->id}}">{{$thincloud->name}} {{$thincloud->surname}}</label>
        <br />
          </div>
          @endforeach
          @if ($errors->useradd->first("thincloud_id") != "")
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" id="modalRoleName" name="name"
                                            ...
                                            <div class="alert alert-danger mt-1 alert-validation-msg" role="alert">
                                                <div class="alert-body d-flex align-items-center">
                                                    <i data-feather="info" class="me-50"></i>
                                                    <span>{{$errors->create->first("name")}}</span>
                                                </div>
                                            </div>
                                            @endif
                                ...
                                <form id="editRoleForm" class="row" action="/roles/update" method="POST">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" id="modalRoleName" name="name" value="{{$role->name}}"
                                            class="form-control" placeholder="Rol ismini gir" tabindex="-1"
                                            data-msg="Lütfen isim gir" />
                                            @if ($errors->update->first("name") != "")
                                            <div class="alert alert-danger mt-1 alert-validation-msg" role="alert">
                                                <div class="alert-body d-flex align-items-center">
                                                    <i data-feather="info" class="me-50"></i>
                                                    <span>{{$errors->update->first("name")}}</span>
                                                </div>
                                            </div>
                                            @endif
                                    </div>
...
@section('js')
  ...
  @endif
  <script>
    $(".role-useradd-modal").click(function (e) {
        var userrolemodalname = $(this).attr("data-bs-target");
        localStorage.setItem("key2", userrolemodalname);
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            var userrolemodalname= localStorage.getItem("key2");
             $(userrolemodalname).modal('show');
        });
    </script>
  @endif
    {{-- <script>
        $(function () {
            $(".role-add-modal").click(function (e) {
                var id = $(this).attr("id");
                $(".get_id").val(id);
                var name = $(this).attr("title");
                $(".address-title").text(name + " Yeni Kullanıcı Ekle");
            });
        });
    </script> --}}
  <script>
