...
class FirewallController extends Controller
{
    public function showpage()
    {
        ...
        $firewallcheck = Firewall::where('user_id', auth()->user()->id)->first();
        $fireDepart = FirewallDepartment::get();
        $fireVm = FirewallVm::get();

        return view('pages.firewall', compact('departman', 'virtual', 'ports', 'firewall', 'fireDepart', 'fireVm', 'firewallcheck'));
    }

    public function create_rule(Request $request){
        $formFields = $request->validateWithBag("createrule",[
    ...
    public function open_ports(Request $request){
        $formFields = $request->validateWithBag("openports",[
